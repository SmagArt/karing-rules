...
### iOS — импорт файлом через «Файлы» (рабочий путь)

Раздел в приложении: **Правила перенаправления → Личные правила** (тот, где список групп).

Файл берётся прямо из iCloud, репозиторий для этого не нужен:

```
Файлы → iCloud Drive → Obsidian → Claude_full → Projects_lite → karing-rules
        → diversion_rules_custom.json
```

Это и есть источник истины — та же папка, что на компьютере, синкается автоматически. Импортировать поверх существующих групп — порядок изменённых съедет в хвост; надёжнее удалить все правила и импортировать начисто (проверено 09.09.2026).

### Импорт по URL — не найден

Ссылка на raw-версию файла существует и отдаёт актуальный JSON:
```
https://raw.githubusercontent.com/SmagArt/karing-rules/main/diversion_rules_custom.json
```
Но **куда её вставлять в Karing — неизвестно.** В официальной документации (README_ru, FAQ karing.app) импорт правил по URL не описан вовсе, упомянут только импорт/экспорт ZIP. Прежняя запись здесь («Настройки → Маршрутизация → Импортировать → URL») пункта с таким названием в приложении не имеет — считать её неверной, пока путь не найден и не проверен руками. Ссылку держим для подписок и для машин, где есть git.
